...
//! tree), unlike list nesting which it caps at 100 — and afm walks that
...
use afm_markdown::{Options, render_blocks_to_ir, render_to_ir, render_to_string, serialize};
...
fn render_to_string_survives_deep_blockquote_nesting() {
    let out = render_to_string(&deeply_nested_blockquotes(), &Options::afm_default());
    assert!(
...
fn render_to_ir_survives_deep_blockquote_nesting() {
    ...
    let rendered = render_to_ir(&deeply_nested_blockquotes(), &Options::afm_default());
...
fn render_blocks_to_ir_survives_deep_blockquote_nesting() {
    let (blocks, _diagnostics) =
        render_blocks_to_ir(&deeply_nested_blockquotes(), &Options::afm_default());
...
fn deep_nesting_with_aozora_annotation_holds_tier_a() {
    ...
    let out = render_to_string(&input, &Options::afm_default());
    assert!(
