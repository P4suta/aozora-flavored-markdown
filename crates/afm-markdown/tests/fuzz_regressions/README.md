...
`crates/afm-markdown/fuzz/`. The integration test
...
2. If libFuzzer flags a crash, it writes the offending input to
   `crates/afm-markdown/fuzz/artifacts/<target>/crash-<sha>` and
   exits non-zero.
...
`afm_markdown_test_support::assert_html_invariants(src, &html)`, which
