...
fn parse_render_regressions_replay_cleanly() {
    replay_each(
        "parse_render",
        |src| {
            let html = render_to_string(src, &Options::afm_default()).html;
...
fn sjis_decode_regressions_replay_cleanly() {
    replay_each(
        "sjis_decode",
        |text| {
            let html = render_to_string(text, &Options::afm_default()).html;
...
fn regression_dir(target: &str) -> PathBuf {
    // CARGO_MANIFEST_DIR resolves to `crates/afm-markdown/`; the test
