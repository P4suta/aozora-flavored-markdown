...
//!
//! Wraps `comrak::format_html`. The Aozora extension's per-node renderer
//! ([`aozora_parser::aozora::html::render`]) has been registered on the
//! `Options` used at parse time, so comrak's own HTML renderer will
//! dispatch `NodeValue::Aozora(_)` arms through to it.
