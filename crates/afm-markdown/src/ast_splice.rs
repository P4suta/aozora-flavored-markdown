    ...
    fn split_text_node(&mut self, node: &'a AstNode<'a>, text: &str) {
        ...
        while let Some(ch) = chars.next() {
            if is_sentinel_char(ch) {
                ...
                html.push_str("<span class=\"afm-annotation\" hidden>");
...
fn render_aozora_html(node: AozoraNode<'_>, entering: bool) -> String {
    ...
    // its own `aozora-*` brand; afm's HTML uses `afm-*`. The rewrite
...
mod tests {
    ...
    /// Run the full afm pipeline (mask → lex → parse → splice →
