...
5. `.github/workflows/release.yml` (generated by `dist`) reacts to
   ...
   licences + `README.md`, generates `shell`/`powershell` installer
   scripts and checksums, and creates the GitHub Release with notes
   derived from the changelog.
6. Sanity check: download one artefact, verify its `.sha256`, then
