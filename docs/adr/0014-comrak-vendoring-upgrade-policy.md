...
pure tree replace (afm carries no comrak patches). What ADR-0001 did *not*
pin down is *when* we follow a new comrak release, how a sync interacts with
releases, and — now that afm-markdown is published (ADR-0015) — how the
vendored tree relates to the comrak version afm depends on.
...
carries a CommonMark/GFM conformance fix afm's spec suite would benefit from.
...
CommonMark/GFM input is a breaking change for afm consumers and bumps the
breaking axis per ADR-0015.

**Published-dep coupling.** The registry `comrak` version afm-markdown
...
- Publishing against the registry comrak (rather than shipping a fork) keeps
  afm-markdown a normal crates.io citizen while preserving the vendored tree
