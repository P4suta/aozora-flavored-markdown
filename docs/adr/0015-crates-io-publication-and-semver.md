...
afm should be installable as a normal Rust crate (`cargo install afm-cli`,
`cargo add afm-markdown`) rather than only via `--git`. Two manifest facts
blocked that: `afm-markdown` depended on `aozora` by **git rev** and on
...
- `comrak`: keep `{ version = "0.52.0", path = "upstream/comrak" }`. cargo uses
  ...
  gate (ADR-0014) keeps them identical, so published afm-markdown builds
  against registry comrak 0.52.0.
- `afm-markdown-test-support`: path-only (no version) so `cargo publish` strips
...
**Publishable set & order.** Only `afm-markdown` then `afm-cli` are published,
in that topological order. `afm-wasm` (npm/wasm-pack), `afm-markdown-test-support`
and `xtask` (dev-only) are `publish = false`; `afm-book` is not a crate.
...
13-crate machinery scaled down — afm's single-library workspace needs only two
rungs.
...
`afm.diagnostics.v1` schema (ADR-0012), the public IR enums (ADR-0013), or
`Options::afm_default`. Patch = additive features and fixes.
...
- `cargo install afm-cli` / `cargo add afm-markdown` become real; docs.rs will
...
- A first publish is greenfield (both crates 404 today); the leaf
  `afm-markdown` dry-run is the pre-flight gate (verified green), while
  `afm-cli` can only be verified live after afm-markdown lands.

## Alternatives considered

**Keep the `aozora` git pin and don't publish.** Rejected: it is the single
reason afm-markdown can't go on crates.io, and aozora is already published, so
